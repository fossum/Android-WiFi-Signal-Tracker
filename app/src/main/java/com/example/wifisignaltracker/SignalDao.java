...
public interface SignalDao {
    ...
    @Query("SELECT DISTINCT ssid FROM measurements WHERE latitude BETWEEN :minLat AND :maxLat AND longitude BETWEEN :minLon AND :maxLon")
    List<String> getSsidsInBounds(double minLat, double maxLat, double minLon, double maxLon);
